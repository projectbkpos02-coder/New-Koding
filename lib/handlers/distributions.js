...
exports.createDistribution = async (req, res) => {
  try {
    ...
    res.json({ message: 'Distribusi berhasil dilakukan' });
  } catch (error) {
    console.error('createDistribution error:', error);
    res.status(error.status || 500).json({ error: error.message || 'Gagal melakukan distribusi' });
  }
};
...
exports.getDistributions = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal mengambil data distribusi' });
  }
};
