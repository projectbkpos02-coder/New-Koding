...
exports.getRiders = async (req, res) => {
  try {
    ...
    // Visibility rules:
    // - super_admin sees all riders (NOT super_admin or admin)
    // - admin sees only riders (and NOT super_admin)
    // - rider cannot access this endpoint
    
    // Filter to show only 'rider' role
    result = result.filter(u => (u.role || 'rider') === 'rider');

    if (requester.role === 'admin') {
      // admin can see only riders
      return res.json(result || []);
    } else if (requester.role === 'super_admin') {
      // super_admin can see all riders
      return res.json(result || []);
    } else {
      // rider cannot access
...
exports.updateRole = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal mengubah role user' });
  }
};
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal menghapus user' });
  }
};
...
exports.getRiderLeaderboard = async (req, res) => {
  try {
    ...
    console.error('getRiderLeaderboard error:', error);
...
exports.getUserReports = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal memuat laporan user' });
  }
};
