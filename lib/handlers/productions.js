...
exports.createProduction = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal merekam produksi' });
  }
};
...
exports.getProductions = async (req, res) => {
  try {
    ...
    res.status(error.status || 500).json({ error: error.message || 'Gagal memuat data produksi' });
  }
};
