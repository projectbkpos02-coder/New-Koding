...
exports.getSummary = async (req, res) => {
  try {
    ...
    // Riders can only see their own summary, admins can see all or specific riders
    ...
    if (start_date) {
      // Handle YYYY-MM-DD format by converting to ISO timestamp
      const startDateTime = start_date.includes('T') ? start_date : `${start_date}T00:00:00Z`;
      query = query.gte('created_at', startDateTime);
    }
    if (end_date) {
      // Handle YYYY-MM-DD format by converting to ISO timestamp (end of day)
      const endDateTime = end_date.includes('T') ? end_date : `${end_date}T23:59:59Z`;
      query = query.lte('created_at', endDateTime);
    }
    
    // Always filter by rider_id
...
// Get leaderboard (all riders can view)
...
exports.getDetailedReport = async (req, res) => {
  try {
    ...
    // Admin can view all reports, riders can only view their own detailed reports
