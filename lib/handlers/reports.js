...
// Generate Excel data (TSV format for Excel compatibility)
exports.exportReportsExcel = async (req, res) => {
  try {
    ...
    // Build Excel content with multiple sections
    let excelContent = '';

    // Title
    excelContent += 'LAPORAN PENJUALAN POS RIDER\n';
    excelContent += `Periode: ${start_date || 'Semua'} - ${end_date || 'Sekarang'}\n`;
    excelContent += `Tanggal Cetak: ${new Date().toLocaleString('id-ID')}\n\n`;

    // Summary
    excelContent += 'RINGKASAN PENJUALAN\n';
    excelContent += 'Keterangan\tJumlah\n';
    excelContent += `Total Transaksi\t${transactions?.length || 0}\n`;
    excelContent += `QRIS\t${qris_total}\n`;
    excelContent += `Tunai\t${tunai_total}\n`;
    excelContent += `Total Penjualan\t${total_sales}\n`;
    excelContent += `Total Biaya/HPP\t${total_cost}\n`;
    excelContent += `Gross Profit\t${gross_profit}\n`;
    excelContent += `Profit Margin\t${profit_margin.toFixed(2)}%\n\n`;

    // Payment Breakdown
    excelContent += 'RINGKASAN METODE PEMBAYARAN\n';
    excelContent += 'Metode\tJumlah Transaksi\tTotal\n';
    const qris_count = (transactions || []).filter(t => t.payment_method === 'qris').length;
    const tunai_count = (transactions || []).filter(t => t.payment_method === 'tunai').length;
    excelContent += `QRIS\t${qris_count}\t${qris_total}\n`;
    excelContent += `Tunai\t${tunai_count}\t${tunai_total}\n\n`;

    // Rider Breakdown
    excelContent += 'RINGKASAN PER RIDER\n';
    excelContent += 'No\tNama Rider\tTransaksi\tQRIS\tTunai\tTotal Penjualan\tBiaya/HPP\tProfit\tMargin\n';
    const riders = Object.values(rider_breakdown).sort((a, b) => b.total_sales - a.total_sales);
    riders.forEach((rider, index) => {
      const profit = rider.revenue - rider.cost;
      const margin = rider.revenue > 0 ? (profit / rider.revenue * 100) : 0;
      excelContent += `${index + 1}\t${rider.rider_name}\t${rider.transactions_count}\t${rider.qris}\t${rider.tunai}\t${rider.total_sales}\t${rider.cost}\t${profit}\t${margin.toFixed(2)}%\n`;
    });

    excelContent += '\n';

    // Transaction Details
    excelContent += 'DETAIL TRANSAKSI\n';
    excelContent += 'No\tTanggal\tRider\tMetode\tTotal\tProfit\tMargin\n';
    (transactions || []).forEach((t, index) => {
      ...
      excelContent += `${index + 1}\t${new Date(t.created_at).toLocaleString('id-ID')}\t${t.profiles?.full_name || '-'}\t${t.payment_method || '-'}\t${t.total_amount}\t${t_profit.toFixed(0)}\t${t_margin.toFixed(2)}%\n`;
    });

    // Set headers for Excel download (TSV format - Excel compatible)
    const filename = `Laporan_Penjualan_${new Date().toISOString().split('T')[0]}.tsv`;
    res.setHeader('Content-Type', 'text/tab-separated-values; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    
    // Convert to Buffer with UTF-8 BOM for Excel
