...
exports.createStockOpname = async (req, res) => {
  try {
    ...
    const opnameRecord = {
      ...
      notes,
      details: { sales_details: salesDetails },
...
exports.getStockOpnames = async (req, res) => {
  try {
    ...
    let query = supabase
      .from('stock_opname')
      .select('id, rider_id, admin_id, total_sales, details, payment_method, created_at');
    ...
    const ids = Array.from(new Set([...riderIds, ...adminIds]));
    let profilesMap = {};
    if (ids.length > 0) {
    ...
    const enriched = rows.map(r => ({
      ...r,
      rider: { full_name: profilesMap[r.rider_id] || null },
      admin: { full_name: profilesMap[r.admin_id] || null }
    }));
