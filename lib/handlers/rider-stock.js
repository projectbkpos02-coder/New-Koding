...
exports.getRiderStock = async (req, res) => {
  try {
    ...
    console.error('getRiderStock error:', error);
    res.status(error.status || 500).json({ error: error.message || 'Gagal mengambil data stok rider' });
  }
};
...
exports.getRiderStockById = async (req, res) => {
  try {
    ...
    console.error('getRiderStockById error:', error);
    res.status(error.status || 500).json({ error: error.message || 'Gagal mengambil data stok rider' });
  }
};
